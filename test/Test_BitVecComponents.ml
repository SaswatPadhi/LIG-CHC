...
let bveq_eval = (List.find_exn BitVecComponents.equality
                   ~f:(fun comp -> String.equal comp.name "bv-eq")).evaluate
let bvadd_eval = (List.find_exn BitVecComponents.octagons
                    ~f:(fun comp -> String.equal comp.name "bvadd")).evaluate
let bvnot_eval = (List.find_exn BitVecComponents.bitwise
                    ~f:(fun comp -> String.equal comp.name "bvnot")).evaluate
let bvult_eval = (List.find_exn BitVecComponents.unsigned_intervals
                    ~f:(fun comp -> String.equal comp.name "bvult")).evaluate
let bvuge_eval = (List.find_exn BitVecComponents.unsigned_intervals
                    ~f:(fun comp -> String.equal comp.name "bvuge")).evaluate
let bvugt_eval = (List.find_exn BitVecComponents.unsigned_intervals
                    ~f:(fun comp -> String.equal comp.name "bvugt")).evaluate
let bvule_eval = (List.find_exn BitVecComponents.unsigned_intervals
                    ~f:(fun comp -> String.equal comp.name "bvule")).evaluate
let bvsub_eval = (List.find_exn BitVecComponents.octagons
